	...
	public void remove(Articulo articuloEnPedido, int unidades) {
			...
			for(int i = 0; i<listaPedido.size();i++) {	
				if(listaPedido.get(i).getCodigo() == articuloEnPedido.getCodigo()) {	
					...
					//Miro cuantas unidades hay del articulo en el pedido
					int unidadesDelPedido = listaPedido.get(i).getUnidades();

					// Si hay menos unidades de las que quieres eliminar	
					
					if(unidadesDelPedido<=unidades) {
						listaPedido.remove(i);
					}
					
					// Mas unidades de las que quieres eliminar 
					else {					
						//Calculo cuantas unidades seran las nuevas
						int unidadesNuevas = unidadesDelPedido - unidades ;
