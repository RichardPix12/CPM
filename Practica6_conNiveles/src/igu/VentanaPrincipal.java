...
public class VentanaPrincipal extends JFrame {
	...
	private JRadioButtonMenuItem mnRbFacil;
	private JRadioButtonMenuItem mnRbIntermedio;
	private JRadioButtonMenuItem mnRbDificil;
	...
	private void reiniciarJuego() {
		...
		//1.1 reiniciar juego
		juego.inicializarJuego(juego.getNivel());
		//2.Interfaz
			
			//2.1 deshabilitar tablero
			habilitaTablero(false);
			
			//2.2 Limpiar los disparos
			despintaDisparos();
	...
	private JLabel getLbPlaneta() {
		if (lbPlaneta == null) {
			...
			lbPlaneta.setBounds(824, 0, 185, 175);
		}
		return lbPlaneta;
	}
	private JTextField getTxPuntos() {
		if (txPuntos == null) {
			...
			txPuntos.setBounds(649, 53, 97, 36);
	...
	private JLabel getLbPuntos() {
		if (lbPuntos == null) {
			...
			lbPuntos.setBounds(649, 20, 97, 27);
		}
		return lbPuntos;
	}
	private JPanel getPnDisparos() {
		if (pnDisparos == null) {
			...
			pnDisparos.setBounds(137, 117, 490, 80);
		}
		return pnDisparos;
	}
	private JPanel getPnTablero() {
		if (pnTablero == null) {
			...
			pnTablero.setBounds(36, 226, 990, 98);
	...
	private JButton nuevaCasilla(int i) {
		JButton bt0;
			bt0 = new JButton("");
			bt0.setActionCommand(String.valueOf(i));
	...
	*/
	...
	private JMenu getMnNivel() {
		if (mnNivel == null) {
			...
			mnNivel.add(getMnRbFacil());
			mnNivel.add(getMnRbIntermedio());
			mnNivel.add(getMnRbDificil());
		}
		return mnNivel;
	}
	...
	private JRadioButtonMenuItem getMnRbFacil() {
		if (mnRbFacil == null) {
			mnRbFacil = new JRadioButtonMenuItem("Facil");
			buttonGroup.add(mnRbFacil);

			
			mnRbFacil.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					inicializar(1);
				}
			});
	...
	private JRadioButtonMenuItem getMnRbIntermedio() {
		if (mnRbIntermedio == null) {
			mnRbIntermedio = new JRadioButtonMenuItem("Intermedio");
			buttonGroup.add(mnRbIntermedio);
			mnRbIntermedio.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					inicializar(2);
				}
			});
			mnRbIntermedio.setSelected(true);
		}
		return mnRbIntermedio;
	}
	private JRadioButtonMenuItem getMnRbDificil() {
		if (mnRbDificil == null) {
			mnRbDificil = new JRadioButtonMenuItem("Dificil");
			buttonGroup.add(mnRbDificil);
			mnRbDificil.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					inicializar(3);
				}
			});
	...
	private void inicializar(int nivel) {
		...
		//2.Interfaz
			...
			// Preparar el tablero
			prepararTablero();
			// Crear botones e inicializar
			creaBotonesYRedimensiona();
			// Mostrar puntos
			getTxPuntos().setText(String.valueOf(juego.getPuntos()));
			// habilitar dado
			getBtDado().setEnabled(true);
	...
	public void creaBotonesYRedimensiona(){
		
		
		
		//Crear tantos botones como casillas tenga el tablero
		for(int i=0; i<juego.getDimensiones();i++) {
			System.out.println("a");
			getPnTablero().add(nuevaCasilla(i));
			//((JButton)(getPnTablero().getComponents()[i])).setActionCommand(String.valueOf(i));
		}
		habilitaTablero(false);
		getPnTablero().validate();
		
	}
	private void prepararTablero() {
		getPnTablero().removeAll();
		getPnTablero().setLayout(new GridLayout(1, juego.getDimensiones(), 4, 0));
		getPnDisparos().removeAll();
		
		if(juego.getNivel() == 1) {
			getPnTablero().setBounds(20, 208, 1010, 98);
		}
		else if(juego.getNivel() == 2) {
			getPnTablero().setBounds(20, 208, 815, 98);
		}
		else {
			getPnTablero().setBounds(20, 208, 610, 98);
		}
